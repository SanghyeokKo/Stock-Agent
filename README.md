...
- **모의투자 환경 의존**: 한국투자증권 모의투자 API는 일부 TR(해외 잔고 등)의
  지원 범위가 실전과 다르고, 토큰 발급 rate limit(분당 1회)이 존재합니다.
  토큰 파일 캐싱으로 완화했으나 다중 프로세스 환경에서는 별도 토큰 스토어가
  필요합니다.
  → **개선 방향**: Phase 2에서 시세 조회용 브로커를 토스증권 API로 분리하여
  KIS 트래픽 집중 문제를 완화하고, 토큰을 Redis 등 외부 스토어에서 관리합니다.

- **In-Memory Checkpointer**: `MemorySaver`는 프로세스 재시작 시 대화 이력이
  소실됩니다.
  → **개선 방향**: `PostgresSaver`로 교체하면 인터페이스 변경 없이 대화 이력이
  영속화됩니다. `MemorySaver()`를 `PostgresSaver(conn)`으로 한 줄만 바꾸면
  되도록 설계되어 있습니다.

- **라우터의 단발 분류**: "시세 조회 후 지식 설명"처럼 복합 의도를 가진 질문은
  현재 단일 intent로 축약되어, 두 번째 의도가 처리되지 않을 수 있습니다.
  → **개선 방향**: LangGraph 서브그래프로 멀티스텝 플래너를 구현하여 의도를
  순서대로 처리하는 방식으로 확장합니다.

- **RAG 데이터셋 필터링 비용**: 매 실행 시 전체 44,870행을 순회해 한국은행
  자료(약 979개)로 필터링하는 오버헤드가 있습니다.
  → **개선 방향**: 필터링 결과를 로컬 JSON 캐시로 저장하여 첫 실행 이후에는
  순회 없이 즉시 로드하도록 최적화합니다.

- **RAG 커버리지**: 현재는 한국은행 경제금융용어 700선 카테고리만 활성화되어
  있어 시사 이슈나 최신 규제 변화는 다루지 못합니다.
  → **개선 방향**: `TARGET_REFERENCES`에 시사경제용어사전 등 다른 소스를
  추가하거나, Tavily 뉴스 검색과 RAG를 결합한 하이브리드 답변 전략을 도입합니다.

- **포트폴리오 노드의 실시간 데이터 미반영**: 현재 포트폴리오 제안은 Pydantic
  구조화 출력 요구사항 구현에 집중되어 있으며, LLM의 사전 학습 지식 기반으로
  동작합니다. 실시간 시세·계좌 잔고·뉴스를 반영하지 않으므로 실제 투자 판단
  근거로 사용해서는 안 됩니다.
  → **개선 방향**: Phase 2에서 실시간 잔고 조회 → 뉴스 분석 → 시세 데이터를
  에이전트가 순서대로 수집한 뒤 리밸런싱을 제안하는 멀티스텝 플래닝으로
  고도화할 예정입니다.
